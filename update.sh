#!/usr/bin/env bash
# Brings UwULock Server to the newest version, where it runs.
#
...
#
#   --dir DIR          where UwULock Server lives (default: where this script is, then /opt/uwulock)
...
# Of the .env it only ever changes what it says it changes.
# The whole story: docs/deployment.md.
set -uo pipefail

repo=MinifyX/UwULock-Server
releases="https://github.com/$repo/releases"
image=ghcr.io/minifyx/uwulock-server
service=uwulock
...
# ── where UwULock Server lives ────────────────────────────────────────────────────────────────
# Next to this script, or /opt/uwulock, or what --dir says — never wherever the shell happens to
# be. "sudo bash /opt/uwulock/update.sh" from somewhere else means that one. And only a directory
# whose compose.yaml runs this server — never another project's.
ours() { [ -f "$1/compose.yaml" ] && [ -f "$1/.env" ] && grep -q "uwulock-server" "$1/compose.yaml"; }
if [ -z "$dir" ]; then
  for candidate in "$here" /opt/uwulock; do
    if [ -n "$candidate" ] && ours "$candidate"; then
      dir="$candidate"
      break
    fi
  done
  [ -n "$dir" ] || die "no UwULock Server here. Pass --dir with the directory it runs from."
fi
dir="$(cd "$dir" 2>/dev/null && pwd -P)" || die "there is no directory $dir"
ours "$dir" || die "$dir holds no compose.yaml for UwULock Server with an .env next to it"

# What is in that directory runs as root: Compose starts whatever compose.yaml and .env say, and
# .uwulock-update decides whether compose.yaml is replaced. So all of it has to belong to root, or
...
for name in compose.yaml .env .uwulock-update $override_names; do
...
while [ "$above" != / ]; do
  above=$(dirname "$above")
  owned_right "$above" above || untrusted "$above" "Update from a directory that only root may change, like /opt/uwulock."
done
...
if grep -qE '^[[:space:]]*(export[[:space:]]+)?(COMPOSE_|DOCKER_)' "$dir/.env"; then
  die "$dir/.env sets COMPOSE_ or DOCKER_ variables, which steer Compose itself rather than UwULock Server. Take them out, then run this again."
fi
...
lifted="$lifted UWULOCK_TRUST_FORWARDED UWULOCK_UPDATE_CHECK"
...
if $backup; then
  if running "$service"; then
    ...
    if written=$(docker exec "$service" uwulock-server backup </dev/null); then
      backup_name=$(printf '%s' "$written" | grep -oE 'uwulock-[0-9-]+\.db' | tail -1)
    ...
    warn "UwULock Server is not running, so nothing was backed up"
  fi
fi
...
if ! $healthy; then
  ...
  if looks_like_version "$running_version"; then
    set_env UWULOCK_VERSION "$running_version"
  elif [ -n "$running_image" ]; then
    docker tag "$running_image" "$image:rollback" >/dev/null 2>&1 && set_env UWULOCK_VERSION rollback
  fi
  docker compose up -d </dev/null >/dev/null 2>&1
  if wait_healthy; then
    warn "UwULock Server runs on the version from before, and .env says which one that is."
    ...
    if docker compose run --rm -T "$service" restore "$backup_name" </dev/null >/dev/null &&
      ...
      warn "UwULock Server runs on the version from before, with the database from just before"
...
if [ -n "$new_version" ] && [ "$new_version" != "$running_version" ]; then
  printf '  UwULock Server is on %s now (=^･ω･^=)\n\n' "$new_version"
else
  printf '  UwULock Server is up to date (=^･ω･^=)\n\n'
fi
