...
//! does not come up. A password server that can replace itself from the network is
...
pub fn build() -> Build {
    Build {
        ...
        commit: option_env!("UWULOCK_GIT_SHA").filter(|sha| !sha.is_empty()),
        release: option_env!("UWULOCK_RELEASE").is_some_and(|tag| !tag.is_empty()),
    }
}
...
impl Channel {
    /// From the tag the machine follows (`UWULOCK_VERSION` in `.env`, handed in
    /// as `UWULOCK_CHANNEL`), and from what is running: a beta that is told
    /// about stable releases only would hear nothing until the next one.
    pub fn of(followed: Option<&str>, running: &str) -> Self {
        if followed.is_some_and(|tag| tag.trim().eq_ignore_ascii_case("beta")) || running.contains('-') {
            Channel::Beta
        } else {
            Channel::Stable
        }
...
pub fn newest_release<'a>(list: &'a [GitHubRelease], current: &str, channel: Channel) -> Option<&'a GitHubRelease> {
    list.iter()
        ...
        loop {
            ...
            match found {
                ...
                Ok(Finding::Release { version, url }) => tracing::info!(
                    ...
                    %url,
                    "UwULock Server {version} is out. To update: sudo bash update.sh, next to compose.yaml"
                ),
...
mod tests {
    ...
    fn the_channel_follows_the_tag_and_what_runs() {
        ...
        assert_eq!(Channel::of(Some("latest"), "0.2.0-beta.1"), Channel::Beta, "a beta hears about the next beta");
    }

    #[test]
    fn the_repository_comes_from_the_package() {
        assert_eq!(repository(), "MinifyX/UwULock-Server");
    }
}
